...
const Header = () => {
  ...
  return (
    <div className="flex justify-center items-center h-[300p] p-8 relative">
      <Splide
        className="w-[300px] h-[250px] sm:w-[450px] md:h-[350px] md:w-[600px] lg:w-[800px] "
        options={{
          rewind: true,
          perPage: 1,
      ...
      >
        {movies.map((item) => {
          return (
            <SplideSlide
              className={
                "flex flex-col justify-center gap-3 p-8 relative z-10 mx-[55px] h-[250px] md:h-[350px]"
              }
              style={{
            ...
            >
              <div className="absolute w-4/5 h-[250px] md:h-[350px] top-0 left-0 flex flex-col justify-center z-20 p-[30px]">
                <p className="text-white font-extrabold text-md md:text-4xl text-left mb-4">
                  {item.original_title}
                </p>
                <p className="text-white text-sm md:text-xl text-left line-clamp-3">
                  {item.overview}
                </p>
                <Link
                  to={`/movies/${item.id}`}
